...
配置 `SHIPWITNESS_DRILL_DATABASE_URL` 后，管理员可以对恢复点执行真实恢复演练。目标必须与运行数据库不同，数据库名必须以 `_drill` 或 `_restore_drill` 结尾。演练会清空并覆盖该专用目标，随后核验迁移版本以及工作区、项目、验收任务和审计记录数量；结果不保存数据库地址或凭据，并进入工作区审计链。上线就绪中心要求最近 90 天内至少有一次通过的演练。

...
export function normalizeSteps(input) {
  ...
  return input.map((raw, index) => {
    ...
    if (raw.action === 'goto') {
      ...
      if (raw.action === 'fill' && raw.secretRef) {
        if (!/^[A-Z][A-Z0-9_]{1,63}$/.test(raw.secretRef)) throw Object.assign(new Error(`第 ${index + 1} 步凭据引用无效`), { status: 400 });
        step.secretRef = raw.secretRef;
      } else if (raw.action === 'fill' || raw.action === 'expectText') {
...
async function performStep(page, projectUrl, step, secrets) {
  if (step.action === 'goto') {
  ...
  if (step.action === 'fill') { const value = step.secretRef ? secrets[step.secretRef] : step.value; if (value == null) throw new Error(`验收凭据 ${step.secretRef} 不存在或无权使用`); await page.locator(step.selector).fill(value, { timeout: 8_000 }); return `填写 ${step.selector}${step.secretRef ? '（使用加密凭据）' : ''}`; }
...
export async function executeBrowserRun({ project, run, artifactsDir, allowedOrigins = [], secrets = {}, launch = options => chromium.launch(options) }) {
    ...
    for (let criterionIndex = 0; criterionIndex < run.criteria.length; criterionIndex += 1) {
      ...
      try {
        ...
        for (let stepIndex = 0; stepIndex < steps.length; stepIndex += 1) {
          ...
          try {
            const detail = await performStep(page, project.url, steps[stepIndex], secrets);

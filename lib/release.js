...
export async function verifyReleaseDirectory(folder) {
  ...
  return { valid: true, version: manifest.version, filesVerified: payloadFiles.length, commit: manifest.commit.toLowerCase(), sbom: { format: sbom.bomFormat, specVersion: sbom.specVersion, components: sbom.components.length } };
}

...
export function rotateEncryptedSecrets(data, oldSecret, newSecret) {
  ...
  const mfaSecrets = (data.users || []).flatMap(item => [['mfaSecretEncrypted', item.mfaSecretEncrypted], ['mfaPendingSecretEncrypted', item.mfaPendingSecretEncrypted]].filter(([, value]) => value).map(([field, value]) => ({ item, field, plaintext: decryptSecret(value, oldSecret) })));
  const acceptanceSecrets = (data.acceptanceSecrets || []).filter(item => item.encryptedValue).map(item => ({ item, plaintext: decryptSecret(item.encryptedValue, oldSecret) }));
  ...
  for (const { item, field, plaintext } of mfaSecrets) item[field] = encryptSecret(plaintext, newSecret);
  for (const { item, plaintext } of acceptanceSecrets) item.encryptedValue = encryptSecret(plaintext, newSecret);
  return { workspaces: signingKeys.length, webhooks: webhookSecrets.length, emailDeliveries: emailMessages.length, mfaSecrets: mfaSecrets.length, acceptanceSecrets: acceptanceSecrets.length };
}

...
export function keyFromSecret(value) {
  const encoded = String(value || '').trim(); const key = Buffer.from(encoded, 'base64');
  if (key.length !== 32 || key.toString('base64') !== encoded) throw Object.assign(new Error('SHIPWITNESS_MASTER_KEY 必须是标准 Base64 编码的 32 字节密钥'), { status: 409 });
  return key;
}

...
ShipWitness is under active development. Version `0.4.0-dev.22` includes signed GitHub push and CI Webhooks with persistent replay protection, role-gated repository synchronization with immutable run snapshots, an owner-only deployment-readiness center, reversible project archival, workspace-isolated contract assets and project portfolios, TLS-first encrypted notification queues, one-time invitations, immutable retry history, and lease-based recovery. Its security controls also include password derivation, HttpOnly SameSite cookies, login throttling, role and workspace isolation, session revocation, PostgreSQL backup and restore tooling, tamper-evident audit chains, encrypted signing material, scoped machine keys, signed outbound Webhooks, checksummed release bundles, transactional key rotation, version-bound rollback, and allowlisted acceptance targets. The readiness verdict and synchronized CI state are deployment evidence, not security certifications. Do not expose the current development release directly to the public internet until it is placed behind HTTPS and an independent security review is complete.

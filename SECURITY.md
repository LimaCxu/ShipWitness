...
ShipWitness is under active development. Version `0.4.0-dev.7` includes password authentication, HttpOnly SameSite cookies, login throttling, role checks, workspace isolation, PostgreSQL storage, verified backup/restore tooling, tamper-evident audit chains, encrypted signing material, scoped machine keys, signed outbound webhooks, checksummed release bundles, transactional key rotation, version-bound rollback orchestration, and allowlisted acceptance targets. Do not expose the current development release directly to the public internet until it is placed behind HTTPS and an independent security review is complete.

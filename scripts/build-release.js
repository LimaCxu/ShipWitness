...
const sbom = JSON.parse(await run(process.platform === 'win32' ? 'npm.cmd' : 'npm', ['sbom', '--omit=dev', '--package-lock-only', '--sbom-format=cyclonedx', '--sbom-type=application'], { cwd: staging, env: process.env }));
sbom.metadata ||= {}; sbom.metadata.component ||= {}; sbom.metadata.component.name = pkg.name; sbom.metadata.component.version = pkg.version;
await writeFile(join(staging, 'SBOM.cdx.json'), `${JSON.stringify(sbom, null, 2)}\n`, { mode: 0o644 });

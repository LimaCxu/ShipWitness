...
> 当前版本：`0.4.0-dev.53`。CI 和标签发布现在会构建客户实际获得的 Docker 镜像，并在该镜像内强制执行真实 Chromium、截图证据、发布门禁和 Ed25519 签名卷宗闭环。这避免用源码目录的绿色测试代替交付镜像验收。开发版本仅适合本地或受控试点；正式公网部署仍需稳定版、HTTPS 和一年内独立安全审查。

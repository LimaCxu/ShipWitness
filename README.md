...
> 当前版本：`0.4.0-dev.55`。发布校验器现在对 SBOM 执行生产许可证门禁：每个组件必须有 npm PURL 和已审核的 SPDX 许可证，缺失或未批准许可证会阻断交付。每个发布包同时内置 CycloneDX 1.5 SBOM，CI 和标签发布会在客户 Docker 镜像内执行真实 Chromium 到 Ed25519 签名卷宗的完整闭环。开发版本仅适合本地或受控试点；正式公网部署仍需稳定版、HTTPS 和一年内独立安全审查。
...
交付前运行 `npm run smoke:self`，会在临时工作区中完成真实浏览器断言、截图、发布门禁和签名卷宗校验，结束后自动清理，不会读写现有工作区数据。

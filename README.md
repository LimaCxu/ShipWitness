...
> 当前版本：`0.4.0-dev.38`。登录页支持无账号枚举的邮件找回密码：一次性链接 30 分钟有效，完成后旧会话全部退出，同时保留原有两步验证，避免通过找回密码绕过第二因素。平台同时具备 TOTP 两步验证、登录设备管理、真实浏览器验收、证据返工闭环、团队权限、GitHub 与 CI 同步、上线就绪中心、安全评审、审计与灾备、发布门禁、密钥轮换和精确回退。开发版本仅适合本地或受控试点；正式公网部署仍需稳定版、HTTPS 和一年内独立安全审查。

...
> 当前版本：`0.4.0-dev.49`。发布包会自动绑定完整 Git 提交号，缺失代码来源的包会被校验器拒绝。验收凭据保险箱支持加密保存、有效期、临期提示、原地轮换和过期执行阻断；系统会主动生成临期/过期告警，并把凭据生命周期纳入上线就绪门禁。开发版本仅适合本地或受控试点；正式公网部署仍需稳定版、HTTPS 和一年内独立安全审查。

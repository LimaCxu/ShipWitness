...
> 当前版本：`0.4.0-dev.51`。登录防暴力破解现在具有可验证的时间窗口、自动过期清理、有界内存使用和标准 `Retry-After` 响应。运行服务、健康接口和发布包共用同一版本源，发布包同时自动绑定完整 Git 提交号。开发版本仅适合本地或受控试点；正式公网部署仍需稳定版、HTTPS 和一年内独立安全审查。

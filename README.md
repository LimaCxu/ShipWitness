...
> 当前版本：`0.4.0-dev.43`。管理员可把已校验备份恢复到专用隔离数据库，系统会真实执行 `pg_restore`、核验 Schema 与核心记录并留下脱敏演练证据；目标数据库必须与生产库不同，且名称以 `_drill` 或 `_restore_drill` 结尾。平台同时具备部署配置中心、首次部署向导、用户治理、MFA、真实浏览器验收、GitHub 与 CI 同步、上线就绪中心、安全评审、审计、备份恢复、发布门禁、密钥轮换和精确回退。开发版本仅适合本地或受控试点；正式公网部署仍需稳定版、HTTPS 和一年内独立安全审查。

...
> 当前版本：`0.4.0-dev.40`。首次部署向导会先检查数据存储、持久化主密钥、HTTPS 公开地址和 SMTP，再引导创建组织与首位管理员，并明确区分“受控试点”和“公网候选”。平台同时具备管理员用户治理、安全找回密码、TOTP 两步验证、登录设备管理、真实浏览器验收、证据返工闭环、团队权限、GitHub 与 CI 同步、上线就绪中心、安全评审、审计与灾备、发布门禁、密钥轮换和精确回退。开发版本仅适合本地或受控试点；正式公网部署仍需稳定版、HTTPS 和一年内独立安全审查。

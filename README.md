...
> 当前版本：`0.4.0-dev.37`。成员可绑定标准 TOTP 验证器；登录和接受工作区邀请均需完成第二步验证，恢复码每条只能使用一次，启停会撤销其他设备并写入审计链。平台同时具备登录设备管理、真实浏览器验收、证据返工闭环、团队权限、GitHub 与 CI 同步、上线就绪中心、安全评审、审计与灾备、发布门禁、密钥轮换和精确回退。开发版本仅适合本地或受控试点；正式公网部署仍需稳定版、HTTPS 和一年内独立安全审查。

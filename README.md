...
> 当前版本：`0.4.0-dev.52`。新增隔离的一键交付自验收：用真实 Chromium 运行页面断言、生成截图、通过发布门禁并验证 Ed25519 签名卷宗，全程不触碰现有数据。登录防暴力破解具有可验证时间窗口、自动过期清理和标准 `Retry-After` 响应。开发版本仅适合本地或受控试点；正式公网部署仍需稳定版、HTTPS 和一年内独立安全审查。

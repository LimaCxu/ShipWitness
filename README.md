...
> 当前版本：`0.4.0-dev.54`。每个发布包内置 CycloneDX 1.5 软件物料清单（SBOM），并把生产依赖、产品名称和版本纳入篡改校验；缺失或错配的 SBOM 不能交付。CI 和标签发布同时在客户 Docker 镜像内强制执行真实 Chromium、截图证据、发布门禁和 Ed25519 签名卷宗闭环。开发版本仅适合本地或受控试点；正式公网部署仍需稳定版、HTTPS 和一年内独立安全审查。

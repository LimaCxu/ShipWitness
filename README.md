...
> 当前版本：`0.4.0-dev.30`。撤销邀请、移除成员、永久清理、撤销 Key 和停用 Webhook 使用统一的高风险操作确认流程，永久清理还需输入确认短语；工作区设置按团队、上线门禁、集成和治理分组，并按角色隐藏无权访问的分类；安全发现使用正式整改弹窗登记复测证据或限时风险接受；新建验收流程只使用当前真实项目和后端标准，登录前不再闪现历史原型状态。平台同时具备签名安全整改证据包、外部安全评审闭环、版本支持周期、Coding Agent 扩展 API、GitHub 与 CI 同步、验收任务提交快照、上线就绪中心、项目归档、标准包、项目总览、真实浏览器验收、证据返工闭环、安全工作区、审计与灾备、CI/CD 发布门禁、密钥轮换与回退，以及默认拒绝未授权出站目标的执行边界。开发版本仅适合本地或受控试点；正式公网部署仍需稳定版、HTTPS 和一年内独立安全审查。

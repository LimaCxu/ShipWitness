...
async function loadAccountPanel() {
  ...
  const [workspaces, members, audit, integrity, apiKeys, webhooks, operations] = await Promise.all([api('/api/workspaces'), api('/api/members'), canAudit ? api('/api/audit?limit=30') : [], canAudit ? api('/api/audit/verify') : null, isOwner ? api('/api/api-keys') : [], isOwner ? api('/api/webhooks') : [], canAudit ? api('/api/system/status') : null]);
  ...
  memberList.innerHTML = members.map(item => `<article><span>${escapeHtml(item.name).slice(0, 1).toUpperCase()}</span><div><b>${escapeHtml(item.name)}${item.id === currentSession.user.id ? ' · 我' : ''}</b><small>${escapeHtml(item.email)}</small></div>${isOwner ? `<select data-member-role="${item.membershipId}"><option value="member" ${item.role === 'member' ? 'selected' : ''}>成员</option><option value="approver" ${item.role === 'approver' ? 'selected' : ''}>审批人</option><option value="owner" ${item.role === 'owner' ? 'selected' : ''}>管理员</option></select><button data-remove-member="${item.membershipId}" aria-label="移除 ${escapeHtml(item.name)}">移除</button>` : `<em>${roleLabel(item.role)}</em>`}</article>`).join('');
...
memberList.onclick = async event => { const button = event.target.closest('[data-remove-member]'); if (!button || !confirm('移除后，该成员在当前工作区的会话和机器 Key 会立即失效。确认移除？')) return; try { const result = await api(`/api/members/${button.dataset.removeMember}`, { method: 'DELETE' }); result.self ? location.reload() : await loadAccountPanel(); toast('成员已移除'); } catch (error) { toast(error.message); } };
passwordForm.onsubmit = async event => { event.preventDefault(); try { await api('/api/account/password', { method: 'POST', body: JSON.stringify({ currentPassword: currentPassword.value, newPassword: newPassword.value }) }); passwordForm.reset(); toast('密码已更新，其他会话已退出'); await loadAccountPanel(); } catch (error) { toast(error.message); } };

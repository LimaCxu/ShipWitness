...
async function loadAccountPanel() {
  ...
  const [workspaces, members, audit, integrity, apiKeys, webhooks] = await Promise.all([api('/api/workspaces'), api('/api/members'), canAudit ? api('/api/audit?limit=30') : [], canAudit ? api('/api/audit/verify') : null, isOwner ? api('/api/api-keys') : [], isOwner ? api('/api/webhooks') : []]);
  ...
  if (canAudit) {
    ...
    auditList.innerHTML = audit.map(item => `<article><i></i><div><b>${escapeHtml(({ 'workspace.initialized': '初始化工作区', 'user.login': '用户登录', 'user.logout': '用户退出', 'workspace.created': '创建工作区', 'workspace.selected': '切换工作区', 'member.added': '添加成员', 'project.created': '创建项目', 'project.updated': '更新项目', 'contract.created': '创建验收标准', 'contract.updated': '更新验收标准', 'run.created': '创建验收任务', 'run.started': '开始执行', 'run.completed': '完成验收', 'issue.created': '创建返工单', 'issue.status_changed': '更新返工状态', 'issue.retest_created': '创建定向复验', 'issue.exported': '导出返工单', 'release.decision_recorded': '签署发布决定', 'api_key.created': '创建机器 API Key', 'api_key.revoked': '撤销机器 API Key', 'webhook.created': '创建发布 Webhook', 'webhook.disabled': '停用发布 Webhook', 'webhook.queued': 'Webhook 已入队', 'webhook.delivered': 'Webhook 已送达', 'webhook.failed': 'Webhook 投递失败', 'dossier.signed': '生成签名卷宗' }[item.action] || item.action))}</b><small>${escapeHtml(item.actor?.name || '系统')} · ${new Date(item.at).toLocaleString('zh-CN')}</small></div><code>#${item.sequence}</code></article>`).join('') || '<div class="contract-empty">尚无审计事件</div>';
  }
  toggleAccount(true);
}

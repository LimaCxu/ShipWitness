...
automationSection.insertAdjacentHTML('afterbegin', '<div class="acceptance-secret-section"><div class="section-title"><div><b>验收凭据保险箱</b><small>登录密码等敏感内容加密保存，任务与证据只记录引用名</small></div></div><div id="acceptanceSecretList" class="automation-list"></div><form id="acceptanceSecretForm" class="acceptance-secret-form"><input id="acceptanceSecretName" maxlength="64" placeholder="凭据名，如 LOGIN_PASSWORD" pattern="[A-Za-z][A-Za-z0-9_]{1,63}" required><input id="acceptanceSecretValue" type="password" maxlength="10000" autocomplete="new-password" placeholder="凭据值（保存后不再显示）" required><button>安全保存</button></form><p class="secret-boundary">在填写步骤中使用 <code>{{secret:LOGIN_PASSWORD}}</code>，不要把真实密码写进验收标准。</p><datalist id="acceptanceSecretRefs"></datalist></div>');
...
async function loadAccountPanel() {
  ...
  if (isOwner) {
    acceptanceSecretList.innerHTML = acceptanceSecrets.map(item => `<article><div><b>${escapeHtml(item.name)}</b><small>加密保存 · ${new Date(item.updatedAt).toLocaleString('zh-CN')}</small></div><button data-delete-acceptance-secret="${item.id}">删除</button></article>`).join('') || '<p>尚未保存验收凭据</p>';
  ...
  if (canAudit) {
    ...
    Object.assign(identityAuditLabels, { 'acceptance_secret.created': '创建验收凭据', 'acceptance_secret.deleted': '删除验收凭据' });
...
acceptanceSecretForm.onsubmit = async event => { event.preventDefault(); try { await api('/api/acceptance-secrets', { method: 'POST', body: JSON.stringify({ name: acceptanceSecretName.value, value: acceptanceSecretValue.value }) }); acceptanceSecretForm.reset(); await loadAccountPanel(); toast('验收凭据已加密保存'); } catch (error) { toast(error.message); } };
acceptanceSecretList.onclick = async event => { const button = event.target.closest('[data-delete-acceptance-secret]'); if (!button) return; const name = button.closest('article')?.querySelector('b')?.textContent || '该凭据'; if (!await confirmBusinessAction({ eyebrow: '验收凭据', title: `删除 ${name}？`, description: '引用该名称的验收任务将无法填写对应内容，历史证据不受影响。', confirmLabel: '删除凭据' })) return; try { await api(`/api/acceptance-secrets/${button.dataset.deleteAcceptanceSecret}`, { method: 'DELETE' }); await loadAccountPanel(); toast('验收凭据已删除'); } catch (error) { toast(error.message); } };

...
document.body.insertAdjacentHTML('beforeend', `<div class="auth-gate" id="authGate" hidden><section class="auth-card"><div class="auth-brand"><span>S</span><div><b>ShipWitness</b><small>发布验收台</small></div></div><div class="auth-copy"><span id="authEyebrow">安全工作区</span><h1 id="authTitle">登录 ShipWitness</h1><p id="authDescription">验收证据、返工单和发布决定只对工作区成员可见。</p></div><form id="authForm"><label id="workspaceField" hidden><span>工作区名称</span><input id="authWorkspace" autocomplete="organization" value="我的工作区"></label><label id="nameField" hidden><span>你的姓名</span><input id="authName" autocomplete="name" value="管理员"></label><label id="emailField"><span>邮箱</span><input id="authEmail" type="email" autocomplete="username" required placeholder="owner@example.com"></label><label id="passwordField"><span>密码</span><input id="authPassword" type="password" autocomplete="current-password" minlength="10" required placeholder="至少 10 个字符"></label><label id="mfaCodeField" hidden><span>动态验证码或恢复码</span><input id="authMfaCode" autocomplete="one-time-code" inputmode="numeric" maxlength="20" placeholder="6 位动态验证码"></label><p class="auth-error" id="authError" hidden></p><button type="submit" id="authSubmit">登录</button></form><footer>本地私有部署 · 会话使用 HttpOnly 安全 Cookie</footer></section></div>`);
...
const showAuth = mode => {
  ...
  const setup = mode === 'setup'; workspaceField.hidden = !setup; nameField.hidden = !setup; emailField.hidden = false; passwordField.hidden = false; mfaCodeField.hidden = true; authMfaCode.required = false; authEmail.required = true; authName.required = setup;
...
const showMfaChallenge = challengeToken => { mfaInvitationActive = authMode === 'invite'; mfaChallengeToken = challengeToken; authMode = 'mfa'; workspaceField.hidden = true; nameField.hidden = true; emailField.hidden = true; passwordField.hidden = true; mfaCodeField.hidden = false; authMfaCode.required = true; authMfaCode.value = ''; authEyebrow.textContent = '两步验证'; authTitle.textContent = '确认是你本人'; authDescription.textContent = '输入验证器生成的 6 位动态验证码，或使用一条尚未用过的恢复码。'; authSubmit.textContent = '验证并进入'; authMfaCode.focus(); };
...
authForm.onsubmit = async event => {
  ...
  try {
    const path = authMode === 'setup' ? '/api/setup' : authMode === 'invite' ? `/api/invitations/${invitationToken}` : authMode === 'mfa' ? '/api/login/mfa' : '/api/login';
    const payload = authMode === 'mfa' ? { challengeToken: mfaChallengeToken, code: authMfaCode.value } : authMode === 'invite' ? { password: authPassword.value, ...(!invitationDetails.existingAccount ? { name: authName.value } : {}) } : { email: authEmail.value, password: authPassword.value, ...(authMode === 'setup' ? { workspaceName: authWorkspace.value, name: authName.value } : {}) };
...
async function loadAccountPanel() {
  ...
  const [workspaces, members, audit, integrity, apiKeys, webhooks, operations, retention, auditExports, invitations, emailStatus, emailDeliveries, readiness, githubIntegration, securityReviews, sessions, mfaStatus] = await Promise.all([api('/api/workspaces'), api('/api/members'), canAudit ? api('/api/audit?limit=30') : [], canAudit ? api('/api/audit/verify') : null, isOwner ? api('/api/api-keys') : [], isOwner ? api('/api/webhooks') : [], canAudit ? api('/api/system/status') : null, isOwner ? api('/api/retention') : null, canAudit ? api('/api/audit-exports') : [], isOwner ? api('/api/invitations') : [], isOwner ? api('/api/email/status') : null, isOwner ? api('/api/email-deliveries') : [], isOwner ? api('/api/readiness') : null, canAudit ? api('/api/integrations/github') : null, canAudit ? api('/api/security/reviews') : [], api('/api/account/sessions'), api('/api/account/mfa')]);

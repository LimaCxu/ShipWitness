...
feedbackList.onclick = async event => {
  const manage = event.target.closest('[data-manage-feedback]'); const promote = event.target.closest('[data-promote-feedback]'); const openContract = event.target.closest('[data-open-feedback-contract]');
  if (openContract) { await api(`/api/projects/${openContract.dataset.projectId}/select`, { method: 'POST' }); toggleFeedback(false); await bootstrapBackend(); contractsBtn.click(); return; }
...
async function loadAccountPanel() {
  ...
  if (canAudit) {
    const identityAuditLabels = { 'workspace.renamed': '重命名工作区', 'user.profile_updated': '更新个人资料', 'feedback.created': '提交试点反馈', 'feedback.promoted': '反馈转为验收标准', 'feedback.status_changed': '更新反馈状态', 'feedback.exported': '导出试点反馈' };

...
async function loadAccountPanel() {
  ...
  const [workspaces, members, audit, integrity, apiKeys, webhooks, operations, retention, auditExports, invitations, emailStatus, emailDeliveries, readiness, githubIntegration] = await Promise.all([api('/api/workspaces'), api('/api/members'), canAudit ? api('/api/audit?limit=30') : [], canAudit ? api('/api/audit/verify') : null, isOwner ? api('/api/api-keys') : [], isOwner ? api('/api/webhooks') : [], canAudit ? api('/api/system/status') : null, isOwner ? api('/api/retention') : null, canAudit ? api('/api/audit-exports') : [], isOwner ? api('/api/invitations') : [], isOwner ? api('/api/email/status') : null, isOwner ? api('/api/email-deliveries') : [], isOwner ? api('/api/readiness') : null, canAudit ? api('/api/integrations/github') : null]);

...
async function loadAccountPanel() {
  ...
  const [workspaces, members, audit, integrity, apiKeys, webhooks, operations, retention, auditExports, invitations, emailStatus, emailDeliveries, readiness, githubIntegration, securityReviews, sessions] = await Promise.all([api('/api/workspaces'), api('/api/members'), canAudit ? api('/api/audit?limit=30') : [], canAudit ? api('/api/audit/verify') : null, isOwner ? api('/api/api-keys') : [], isOwner ? api('/api/webhooks') : [], canAudit ? api('/api/system/status') : null, isOwner ? api('/api/retention') : null, canAudit ? api('/api/audit-exports') : [], isOwner ? api('/api/invitations') : [], isOwner ? api('/api/email/status') : null, isOwner ? api('/api/email-deliveries') : [], isOwner ? api('/api/readiness') : null, canAudit ? api('/api/integrations/github') : null, canAudit ? api('/api/security/reviews') : [], api('/api/account/sessions')]);
  ...
  if (canAudit) {
    const identityAuditLabels = { 'workspace.renamed': '重命名工作区', 'user.profile_updated': '更新个人资料', 'user.session_revoked': '退出登录设备', 'feedback.created': '提交试点反馈', 'feedback.promoted': '反馈转为验收标准', 'feedback.status_changed': '更新反馈状态', 'feedback.reopened': '重新打开反馈', 'feedback.verified_by_run': '验收证据关闭反馈', 'feedback.exported': '导出试点反馈' };

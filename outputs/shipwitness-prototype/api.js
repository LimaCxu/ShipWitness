...
document.body.insertAdjacentHTML('beforeend', `<aside id="portfolioPanel" class="portfolio-panel" aria-hidden="true"><header><div><span>发布管理</span><h2>项目总览</h2><p>按真实验收证据汇总当前工作区，不使用 AI 猜测状态。</p></div><button id="closePortfolio" aria-label="关闭">×</button></header><section id="portfolioSummary" class="portfolio-summary"></section><div class="portfolio-toolbar"><div><b>全部项目</b><small id="portfolioUpdated">正在读取…</small></div><button id="portfolioAdd" type="button">＋ 接入项目</button></div><section id="portfolioList" class="portfolio-list"><p>正在汇总项目状态…</p></section></aside><div id="portfolioMask" class="portfolio-mask" hidden></div>`);
let authMode = 'login';
...
async function loadPortfolio() {
  ...
  portfolioSummary.innerHTML = `<article><b>${overview.summary.projects}</b><span>项目总数</span></article><article class="attention"><b>${overview.summary.actionable}</b><span>需要处理</span></article><article><b>${overview.summary.inProgress}</b><span>进行中</span></article><article class="success"><b>${overview.summary.approved}</b><span>已批准</span></article>`;
  portfolioUpdated.textContent = `更新于 ${new Date().toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })}`;
  portfolioList.innerHTML = overview.items.map(item => { const [label, tone] = portfolioState(item.state); return `<article class="portfolio-card ${tone}" data-project-card="${item.id}"><header><div><span>${escapeHtml(item.branch)}</span><h3>${escapeHtml(item.name)}</h3></div><em>${label}</em></header><p>${item.latestRun ? escapeHtml(item.latestRun.requirement) : '还没有创建验收任务'}</p><div class="portfolio-metrics"><span><b>${item.counts.runs}</b> 验收</span><span><b>${item.counts.enabledContracts}</b> 标准</span><span class="${item.counts.openIssues ? 'has-issues' : ''}"><b>${item.counts.openIssues}</b> 待返工</span></div><footer><small>${new Date(item.updatedAt).toLocaleString('zh-CN')}</small><div><button type="button" data-open-project="${item.id}">进入项目</button>${item.latestRun ? `<button type="button" class="primary" data-open-portfolio-run="${item.latestRun.id}" data-project-id="${item.id}">查看任务</button>` : ''}</div></footer></article>`; }).join('') || '<div class="portfolio-empty"><b>还没有项目</b><p>接入第一个项目后，这里会显示它的发布状态。</p></div>';
...
const inboxTypeLabel = type => ({ execution: '等待执行', recovery: '需要接管', failure: '验收失败', approval: '等待审批', retest: '等待复验', delivery: '投递失败' }[type] || '待处理');
...
async function loadAccountPanel() {
  ...
  const [workspaces, members, audit, integrity, apiKeys, webhooks, operations, retention, auditExports, invitations, emailStatus, emailDeliveries] = await Promise.all([api('/api/workspaces'), api('/api/members'), canAudit ? api('/api/audit?limit=30') : [], canAudit ? api('/api/audit/verify') : null, isOwner ? api('/api/api-keys') : [], isOwner ? api('/api/webhooks') : [], canAudit ? api('/api/system/status') : null, isOwner ? api('/api/retention') : null, canAudit ? api('/api/audit-exports') : [], isOwner ? api('/api/invitations') : [], isOwner ? api('/api/email/status') : null, isOwner ? api('/api/email-deliveries') : []]);

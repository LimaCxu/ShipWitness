...
governanceSection.insertAdjacentHTML('beforebegin', '<section id="backupSection" class="backup-section"><div class="section-title"><div><b>备份与恢复</b><small>创建可校验恢复点，恢复前必须停服并完成预检</small></div><button id="createBackup" type="button">创建备份</button></div><div id="backupState" class="backup-state">正在读取…</div><div id="backupList" class="backup-list"></div><div id="backupPreflight" class="backup-preflight" hidden></div></section>');
...
async function loadAccountPanel() {
  ...
  const [workspaces, members, audit, integrity, apiKeys, webhooks, operations, retention, auditExports, invitations, emailStatus, emailDeliveries, readiness, githubIntegration, securityReviews, sessions, mfaStatus, backups, deploymentConfiguration] = await Promise.all([api('/api/workspaces'), api('/api/members'), canAudit ? api('/api/audit?limit=30') : [], canAudit ? api('/api/audit/verify') : null, isOwner ? api('/api/api-keys') : [], isOwner ? api('/api/webhooks') : [], canAudit ? api('/api/system/status') : null, isOwner ? api('/api/retention') : null, canAudit ? api('/api/audit-exports') : [], isOwner ? api('/api/invitations') : [], isOwner ? api('/api/email/status') : null, isOwner ? api('/api/email-deliveries') : [], isOwner ? api('/api/readiness') : null, canAudit ? api('/api/integrations/github') : null, canAudit ? api('/api/security/reviews') : [], api('/api/account/sessions'), api('/api/account/mfa'), isOwner ? api('/api/backups') : null, isOwner ? api('/api/deployment/configuration') : null]);
  ...
  if (canAudit) {
    ...
    Object.assign(identityAuditLabels, { 'backup.created': '创建数据备份', 'backup.verified': '校验备份完整性', 'backup.restore_preflighted': '完成恢复预检' });
...
backupList.onclick = async event => {
  const verify = event.target.closest('[data-verify-backup]'); const preflight = event.target.closest('[data-preflight-backup]');
  try {

...
securityReviewList.onclick = async event => { const button = event.target.closest('[data-security-finding]'); if (!button) return; try { const status = button.dataset.securityStatus; const payload = { status }; if (status === 'verified') { const evidence = prompt('填写独立复测编号或可核验证据'); if (!evidence) return; payload.evidence = evidence; } if (status === 'risk_accepted') { const rationale = prompt('填写临时接受风险的业务原因'); if (!rationale) return; payload.rationale = rationale; payload.expiresAt = new Date(Date.now() + 30 * 86400_000).toISOString(); if (!confirm('该风险将临时接受 30 天，并继续在就绪中心显示警告。确认？')) return; } await api(`/api/security/findings/${button.dataset.securityFinding}`, { method: 'PATCH', body: JSON.stringify(payload) }); await loadAccountPanel(); toast(status === 'verified' ? '复测证据已登记' : status === 'risk_accepted' ? '风险已临时接受 30 天' : '已标记等待复测'); } catch (error) { toast(error.message); } };

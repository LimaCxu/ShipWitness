...
const accountGroupedSections = [
  ...
  [githubIntegrationSection, 'integrations'], [automationSection, 'integrations'],
  [governanceSection, 'governance'], [auditSection, 'governance']
];
...
async function loadFeedback({ open = false, focusId = null } = {}) {
  ...
  feedbackList.innerHTML = items.map(item => `<article class="feedback-card ${item.severity} ${item.id === focusId ? 'focused' : ''}" data-feedback-id="${item.id}"><header><div><span>${feedbackKindLabel(item.kind)} · ${feedbackSeverityLabel(item.severity)}</span><h3>${escapeHtml(item.title)}</h3></div><div class="feedback-card-actions"><em>${feedbackStatusLabel(item.status)}</em>${canManage ? `<button data-manage-feedback="${item.id}" data-feedback-status-value="${item.status}" data-feedback-title="${escapeHtml(item.title)}">处理</button>${item.project && !item.linkedContractId && !['resolved', 'declined'].includes(item.status) ? `<button data-promote-feedback="${item.id}" data-feedback-title="${escapeHtml(item.title)}">转为标准</button>` : ''}${item.linkedContractId ? `<button data-open-feedback-contract="${item.linkedContractId}" data-project-id="${item.projectId}">查看标准</button>` : ''}` : ''}</div></header><p>${escapeHtml(item.description)}</p>${item.verification ? `<div class="feedback-verification"><div><b>✓ 已由验收证据确认解决</b><span>标准 V${item.verification.contractVersion} · ${new Date(item.verification.verifiedAt).toLocaleString('zh-CN')}</span></div><button data-open-feedback-run="${item.verification.runId}" data-project-id="${item.projectId}">查看验证任务</button></div>` : ''}<footer><span>${escapeHtml(item.project?.name || '工作区通用')} · ${escapeHtml(item.reporter?.name || '未知成员')}</span><time>${new Date(item.createdAt).toLocaleString('zh-CN')}</time></footer></article>`).join('') || '<p class="feedback-empty">当前筛选条件下没有反馈。</p>';
...
const closeFeedbackActionDialog = () => { pendingFeedbackAction = null; feedbackActionDialog.close(); };
...
feedbackList.onclick = async event => {
  const manage = event.target.closest('[data-manage-feedback]'); const promote = event.target.closest('[data-promote-feedback]'); const openContract = event.target.closest('[data-open-feedback-contract]'); const openRun = event.target.closest('[data-open-feedback-run]');
  ...
  if (!manage && !promote) return;
  pendingFeedbackAction = { mode: promote ? 'promote' : 'status', id: (promote || manage).dataset.promoteFeedback || manage?.dataset.manageFeedback };
  feedbackActionError.hidden = true; feedbackStatusFields.hidden = Boolean(promote); feedbackPromoteFields.hidden = !promote;
  feedbackActionEyebrow.textContent = promote ? '反馈转化' : '反馈处理'; feedbackActionTitle.textContent = promote ? '创建验收标准草稿' : '更新处理状态'; feedbackActionDescription.textContent = (promote || manage).dataset.feedbackTitle;
...
async function loadAccountPanel() {
  ...
  const [workspaces, members, audit, integrity, apiKeys, webhooks, operations, retention, auditExports, invitations, emailStatus, emailDeliveries, readiness, githubIntegration, securityReviews] = await Promise.all([api('/api/workspaces'), api('/api/members'), canAudit ? api('/api/audit?limit=30') : [], canAudit ? api('/api/audit/verify') : null, isOwner ? api('/api/api-keys') : [], isOwner ? api('/api/webhooks') : [], canAudit ? api('/api/system/status') : null, isOwner ? api('/api/retention') : null, canAudit ? api('/api/audit-exports') : [], isOwner ? api('/api/invitations') : [], isOwner ? api('/api/email/status') : null, isOwner ? api('/api/email-deliveries') : [], isOwner ? api('/api/readiness') : null, canAudit ? api('/api/integrations/github') : null, canAudit ? api('/api/security/reviews') : []]);
  ...
  if (isOwner) {
    ...
    emailDeliveryList.innerHTML = emailDeliveries.slice(0, 8).map(item => `<article><div><b>${escapeHtml(({ workspace_invitation: '成员邀请', release_approval: '发布待审批', run_failed: '验收失败', configuration_test: '配置测试' }[item.kind] || item.kind))}</b><small>${escapeHtml(item.recipient)} · ${item.status === 'delivered' ? '已送达' : item.status === 'failed' ? `失败：${escapeHtml(item.lastError || '')}` : `投递中 · 第 ${item.attempts} 次`}</small></div>${item.status === 'failed' ? `<button data-retry-email="${item.id}">重试</button>` : `<em>${item.status === 'delivered' ? '完成' : '等待'}</em>`}</article>`).join('') || '<p>尚无邮件投递记录</p>';
  }
  auditSection.dataset.accountAllowed = String(canAudit);
  if (canAudit) {
    const identityAuditLabels = { 'workspace.renamed': '重命名工作区', 'user.profile_updated': '更新个人资料', 'feedback.created': '提交试点反馈', 'feedback.promoted': '反馈转为验收标准', 'feedback.status_changed': '更新反馈状态', 'feedback.verified_by_run': '验收证据关闭反馈', 'feedback.exported': '导出试点反馈' };

...
async function loadFeedback({ open = false, focusId = null } = {}) {
  ...
  feedbackList.innerHTML = items.map(item => `<article class="feedback-card ${item.severity} ${item.id === focusId ? 'focused' : ''}" data-feedback-id="${item.id}"><header><div><span>${feedbackKindLabel(item.kind)} · ${feedbackSeverityLabel(item.severity)}</span><h3>${escapeHtml(item.title)}</h3></div>${canManage ? `<select data-feedback-status="${item.id}">${['new', 'triaged', 'planned', 'resolved', 'declined'].map(status => `<option value="${status}" ${status === item.status ? 'selected' : ''}>${feedbackStatusLabel(status)}</option>`).join('')}</select>` : `<em>${feedbackStatusLabel(item.status)}</em>`}</header><p>${escapeHtml(item.description)}</p><footer><span>${escapeHtml(item.project?.name || '工作区通用')} · ${escapeHtml(item.reporter?.name || '未知成员')}</span><time>${new Date(item.createdAt).toLocaleString('zh-CN')}</time></footer></article>`).join('') || '<p class="feedback-empty">当前筛选条件下没有反馈。</p>';
...
feedbackList.onchange = async event => { const select = event.target.closest('[data-feedback-status]'); if (!select) return; try { await api(`/api/feedback/${select.dataset.feedbackStatus}`, { method: 'PATCH', body: JSON.stringify({ status: select.value }) }); await loadFeedback(); await loadInbox(); toast('反馈状态已更新'); } catch (error) { toast(error.message); await loadFeedback(); } };
...
async function loadAccountPanel() {
  ...
  if (canAudit) {
    const identityAuditLabels = { 'workspace.renamed': '重命名工作区', 'user.profile_updated': '更新个人资料', 'feedback.created': '提交试点反馈', 'feedback.status_changed': '更新反馈状态', 'feedback.exported': '导出试点反馈' };

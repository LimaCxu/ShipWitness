...
const renderContracts = () => {
  ...
  contractList.innerHTML = backendContracts.map(item => `<article class="contract-item ${item.enabled ? '' : 'disabled'}" data-id="${item.id}"><div class="contract-item-head"><span>${escapeHtml(item.code)} · V${item.version}</span><em>${item.severity === 'blocker' ? '阻断' : item.severity === 'major' ? '重要' : '一般'}</em></div><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.description)}</p><footer><span>${escapeHtml(item.category)} · ${item.enabled ? '已启用' : '已停用'}</span><div><button data-action="toggle">${item.enabled ? '停用' : '启用'}</button><button data-action="edit">编辑</button></div></footer></article>`).join('') || '<div class="contract-empty">还没有验收标准</div>';
...
async function bootstrapBackend() {
  try {
    ...
    if (runs[0]) {
      ...
      const statusLabel = runs[0].status === 'completed' ? '基础检查完成' : '等待执行';
...
window.shipwitnessCreateRun = async () => {
  ...
  const criteria = backendContracts.filter(item => item.enabled && selectedIds.includes(item.id)).map(item => ({ contractId: item.id, code: item.code, title: item.title, description: item.description, category: item.category, severity: item.severity, version: item.version }));
...
contractEditor.onsubmit = async event => {
  ...
  const payload = { title: contractTitle.value, description: contractDescription.value, category: contractCategory.value, severity: contractSeverity.value };
  try {
...
const resultLabel = value => ({ ready: '已就绪', warning: '注意', failed: '未通过', blocked: '被阻断', evidence_insufficient: '证据不足' }[value] || value);
async function loadRunTask() {
  ...
  runTaskStatus.textContent = run.status === 'completed' ? '基础检查已完成' : run.status === 'running' ? '正在执行' : '等待执行';
  runTaskSummary.textContent = run.execution?.summary || '任务已保存，尚未运行任何检查。';
  backendCriteria.innerHTML = (run.execution?.criteriaResults || run.criteria.map(item => ({ ...item, result: 'queued', reason: '等待基础执行器' }))).map(item => `<article><i class="${item.result}"></i><div><b>${item.title}</b><p>${item.reason || item.description}</p></div><em>${resultLabel(item.result)}</em></article>`).join('');
  if (run.execution) {
...
executeRunBtn.onclick = async () => {
  executeRunBtn.disabled = true; executeRunBtn.textContent = '正在收集证据…'; runTaskStatus.textContent = '正在执行';
  try { await api(`/api/runs/${backendRunId}/execute`, { method: 'POST' }); await loadRunTask(); toast('基础证据已保存到后端'); }
  catch (error) { toast(error.message); }

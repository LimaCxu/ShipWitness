...
const resetAcceptanceSecretForm = () => { acceptanceSecretForm.reset(); acceptanceSecretId.value = ''; acceptanceSecretName.disabled = false; acceptanceSecretSubmit.textContent = '安全保存'; cancelAcceptanceSecretRotation.hidden = true; };
acceptanceSecretForm.onsubmit = async event => { event.preventDefault(); try { const rotating = Boolean(acceptanceSecretId.value); await api(rotating ? `/api/acceptance-secrets/${acceptanceSecretId.value}` : '/api/acceptance-secrets', { method: rotating ? 'PATCH' : 'POST', body: JSON.stringify({ name: acceptanceSecretName.value, value: acceptanceSecretValue.value }) }); resetAcceptanceSecretForm(); await loadAccountPanel(); toast(rotating ? '验收凭据已原地轮换，标准引用保持有效' : '验收凭据已加密保存'); } catch (error) { toast(error.message); } };
